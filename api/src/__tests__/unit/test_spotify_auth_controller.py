"""
Tests for api.controllers.spotify_auth_controller -- Sign in with Spotify,
...
class TestIsConfigured:
    def test_false_when_redirect_uri_missing(self):
        with patch("api.controllers.spotify_auth_controller.settings") as mock_settings:
            mock_settings.SPOTIFY_CLIENT_ID = "client-id"
            mock_settings.SPOTIFY_CLIENT_SECRET = "client-secret"
...
class TestHandleCallback:
    ...
    async def test_redirects_with_error_when_token_exchange_fails(self):
        with patch("api.controllers.spotify_auth_controller.settings") as mock_settings:
            ...
            with patch(
                "api.controllers.spotify_auth_controller.spotify_oauth_client.exchange_code",
    ...
    async def test_links_to_existing_session_when_state_carries_a_subject(self):
        ...
        with patch("api.controllers.spotify_auth_controller.settings") as mock_settings:
            ...
            with patch(
                ...
                "api.controllers.spotify_auth_controller.spotify_oauth_client.fetch_profile",
    ...
    async def test_signs_in_to_an_existing_account_already_linked_by_spotify_id(self):
        ...
        with patch("api.controllers.spotify_auth_controller.settings") as mock_settings:
            ...
            with patch(
                ...
                "api.controllers.spotify_auth_controller.fetch_one",
...
class TestGetLinkedAccount:
    async def test_returns_connected_false_when_not_linked(self):
        with patch("api.controllers.spotify_auth_controller.fetch_one", AsyncMock(return_value=None)):
    ...
    async def test_returns_identity_when_linked(self):
        ...
        with patch("api.controllers.spotify_auth_controller.fetch_one", AsyncMock(return_value=row)):
            result = await spotify_auth_controller.get_linked_account(db=object(), user_uid="USR_X")
...
class TestDisconnect:
    async def test_deletes_the_linked_account(self):
        execute_mock = AsyncMock()
        with patch("api.controllers.spotify_auth_controller.execute", execute_mock):
