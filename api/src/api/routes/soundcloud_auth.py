...
from api.controllers import soundcloud_auth_controller
...
async def status(db, request):
    ...
    if error:
        return error
    account = await soundcloud_auth_controller.get_linked_account(db, user_uid)
...
async def start(db, request):
    ...
    url = soundcloud_auth_controller.build_authorize_url(user_uid)
    if not url:
        return json_error("SoundCloud sign-in is not configured.", 503, code="SOUNDCLOUD_NOT_CONFIGURED")
...
async def callback(db, request):
    ...
    redirect_url = await soundcloud_auth_controller.handle_callback(db, code, state, error)
...
async def disconnect(db, request):
    ...
    if error:
        return error
    await soundcloud_auth_controller.disconnect(db, user_uid)
    return json_response({"status": "success", "message": "SoundCloud disconnected."})
